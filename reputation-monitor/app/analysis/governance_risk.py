...
• Due diligence geopolityczne: jeśli są potwierdzone publiczne powiązania \
  (np. rejestry, media branżowe, listy sankcyjne UE/US/UK) tej osoby z podmiotami \
  w jurysdykcjach wysokiego ryzyka (m.in. Federacja Rosyjska, Białoruś) albo \
  z firmami notowanymi w kontekście obejścia sankcji — użyj kind \
  "foreign_adversary_exposure" z umiarkowaną/ wysoką severity zależnie od \
  wiarygodności źródła. Bez potwierdzenia — nie wymyślaj.
  ...
  "flags": [
    {
      "kind": "past_bankruptcy" | "past_liquidation" | "active_liquidation" | \
              "disqualification" | "news_scandal" | "foreign_adversary_exposure" | "other",
